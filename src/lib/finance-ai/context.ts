...
export function buildFinanceAIPlanningContext(
  ...
  return [
    ...
    "只允许这些动作：metric_snapshot、trend_chart、bar_rank、pareto_rank、small_multiples_trend、waterfall_bridge、grouped_bar、stacked_bar、percent_stacked_bar、heatmap、scatter_bubble、detail_table。",
    ...
    "- pareto_rank 使用 metric、dimension、period、sort、limit；用户说集中度、累计贡献、累计占比、二八或 Pareto 时用它。",
    "- small_multiples_trend 使用 metric、dimension、filters、limit、highlightPeriod；不需要 period，会按所有期间生成多个小趋势面板。",
    ...
    "- 用户没写期间时，metric_snapshot、bar_rank、pareto_rank、grouped_bar、stacked_bar、percent_stacked_bar、heatmap、scatter_bubble、detail_table 默认填最新可用期间 key。",
    ...
    "- pareto_rank 默认 sort:\"value_desc\"；如果用户明确说最低/倒数才用 value_asc。",
