...
export function buildFinanceAIPlanningContext(
  ...
  return [
    ...
    "用户问多个指标是否都增长、均增长、同时增长、都下降或同时变化时，必须用 detail_table，metrics 放入这些指标，comparison:\"mom\"，dimension 放用户询问的维度；解释阶段按每个指标的环比变化列判断交集。",
    ...
    "- detail_table 使用 metrics 数组，最多 6 个指标；scatter_bubble 使用 xMetric/yMetric/sizeMetric。",
    "- dimension 只能使用维度字段里的名称。",
    "- stacked_bar/percent_stacked_bar 使用 dimension 作为横轴主维度，seriesDimension 作为堆叠系列维度。",
