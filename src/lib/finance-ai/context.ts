...
export function buildFinanceAIPlanningContext(
  ...
  return [
    ...
    "waterfall_bridge 只允许用于总额指标；单车指标、均值指标或 per-unit 指标问环比影响/哪些维度影响时，必须用 bar_rank，设置 comparison:\"mom\" 和 period 为当前期。",
    "可用字段：",
    ...
    "- 单车指标问“哪些车型/国家/维度影响环比”时，用 bar_rank，metric 填单车指标，dimension 填对应维度，comparison:\"mom\"，sort 默认 change_desc；不要用 waterfall_bridge。",
