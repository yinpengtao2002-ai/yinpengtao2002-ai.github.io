...
export function getLocalFallbackResponse(
    ...
    if (includesAny(lower, ["lucas", "卢卡斯", "站长", "博主", "作者", "yinpengtao", "殷鹏焘", "殷鹏涛"])) {
        return withOfflineNotice({
            response: "Lucas Yin（殷鹏焘）目前在奇瑞汽车做财务 BP，关注财务建模、数据分析和 AI 工具应用。你可以从 [财务建模](/finance) 或 [AI 见闻](/ai) 开始看。",
    ...
    if (includesAny(lower, ["单车", "边际", "归因", "结构效应", "费率效应", "变动分析", "margin"])) {
        ...
        return withOfflineNotice({
            response: "你要找的是 [单车边际变动归因分析](/finance/margin-analysis)。它用于上传两期财务数据，并按当前维度口径拆解结构效应和费率效应。",
    ...
    if (includesAny(lower, ["ai", "人工智能", "见闻", "chatgpt", "llm", "openai", "gpt", "claude"])) {
        if (aiContent.length === 0) {
            return withOfflineNotice({ response: "【AI 见闻】板块暂时还没有内容，敬请期待。" }, options.includeOfflineNotice);
        }
        return withOfflineNotice({
            response: "这些是【AI 见闻】板块目前可以阅读的内容：",
    ...
    if (includesAny(lower, ["文章", "内容", "有什么", "推荐", "目录", "网站", "看什么", "入口", "板块"])) {
        return withOfflineNotice({
            response: "你可以先看两个入口：[财务建模](/finance) 和 [AI 见闻](/ai)。如果想直接试工具，我推荐打开 [单车边际变动归因分析](/finance/margin-analysis)。",
        }, options.includeOfflineNotice);
    }
