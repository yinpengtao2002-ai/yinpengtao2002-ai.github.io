...
const modelDetails: Record<string, { focus: string; detail: string; guide: string; points: string[] }> = {
  ...
  "profit-structure": {
    focus: "任意维度下的结构诊断",
    detail: "上传通用经营明细后，按大区、国家、渠道、客户、业务单元或任意组合分析销量、单位值和上传指标。",
    guide: "先选当前分析维度和矩阵指标，再看结构矩阵与候选图表，最后回到明细表确认异常对象。",
    points: ["任意维度分析", "动态指标同步", "结构矩阵"],
  },
  ...
  "finance-ai-assistant": {
    ...
    guide: "先上传经营明细，再用自然语言提问；AI 直接读取底稿完成分析，页面负责把趋势、排名或瀑布桥稳定展示出来。",
    points: ["持续聊天问答", "图表嵌入回复", "底稿直接分析"],
  },
};
