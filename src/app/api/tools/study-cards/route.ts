...
const VOCABULARY_CARD_RULES = [
  "目标：生成英文单词或短语卡，帮助用户背英文表达，而不是总结文章观点或出阅读理解题。",
  "逐行单词模式：如果用户每行基本是一个英文单词或短语，保留用户给出的单词或短语，不要替换成别的表达；只补充中文释义、音标、英文例句、例句中文翻译和难度标签。",
  "英文文章模式：如果用户输入的是英文文章，从原文中挑选难度最高且值得记忆的单词或短语；只从原文中出现过的单词或短语里选择，不要发明文章外表达。",
  "文章抽词优先级：优先选择抽象名词、学术词、表达力强的动词/形容词、考试高频词、影响理解的关键词，以及搭配自然、能复用的高价值短语。",
  "短语要求：可以输出 2 到 4 个英文词组成的短语，例如名词短语、动词短语、固定搭配或学术表达；不要输出完整句子。",
  "不要选择专有名词、数字、缩写、过于常见的基础词、语气词、冠词、普通代词或只在原文里无意义重复的词。",
  "同一个单词或短语只出现一次；如果原文有不同词形，优先输出最常见的词典原形或稳定短语，但 source 里保留原文句子。",
...
function buildStudyCardPrompt({
  ...
  return [
    "你是一个严格的英文词汇教练，任务是把用户输入转成可背诵的英文单词或短语卡。",
    "请生成适合逐张翻看的 AI 英文单词或短语卡。正面给英文单词或短语，背面给中文释义、英文例句和来源线索。",
    "输入判断：",
    `- 系统初步判断：${modeHint}`,
    ...
    `- 最多生成 ${cardCount} 张单词或短语卡；如果逐行输入少于这个数量，就按实际数量生成`,
    "- word 只写英文单词或短语，不要写中文，不要加序号；短语通常为 2 到 4 个英文词",
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!hasEnoughVocabularyInput(content)) {
      return Response.json(
        { error: "请至少输入 3 个英文单词/短语，或一段 80 字以上英文文章。" },
        { status: 400 }
      );
    }
