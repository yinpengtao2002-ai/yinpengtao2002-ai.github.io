...
状态：`处理中`（2026-06-27，`.ts` extension import 的类型抑制已移除；财务 `.js` 引擎 TS 化仍后续处理）
...
| P1 | UI | token / 原语库零采用、死代码 | 死代码已清理；token / 原语库采用另行收口 | `ui/*`、`globals.css`、`lib/animations*` |
| P2 | 架构 | 鉴权分散、`.js` 引擎绕过类型、仓库产物堆积 | private-tool access 命名、middleware 私有 API 鉴权、`.ts` import 抑制、本地产物清理命令与私有 HTML 生成物拆分已清；`.js` 引擎 TS 化继续拆分 | 各引擎 `.js`、`tsconfig.json`、`private-tool-access`、`scripts/clean-local-artifacts.mjs`、`src/lib/lucas/stock-decision/stockDecision.html` |
| P2 | UI | 断点不一致 / a11y / 阅读体验 | 导航 landmark、KaTeX/Mermaid、阅读进度与目录 scroll-spy 已修；断点另行评估 | `globals.css`、`ArticleReader.tsx` |
