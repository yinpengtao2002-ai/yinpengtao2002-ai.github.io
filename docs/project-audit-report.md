...
| 2026-06-22 | UI P1-6 | 已修复 | 已修复财务模型卡片移动端“测试中”角标位置：`@media (max-width: 768px)` 下继续锚定卡片右上角，使用 `right: -26px` / `left: auto`，避免角标从左侧偏移压住卡片标题或缩略图。该代码已在提交 `8e7320e` 中进入 `main`。 | `node --test tests/finance-model-registry.test.mjs`；`npx tsc --noEmit`；`npm run lint`；`git diff --check`；`npm run test:site`；`npm run build:vercel`；本地生产 Playwright `390×844` 打开 `/finance` 确认测试角标位于模型卡右上角 |
...
- **财务模型卡片移动端测试角标偏位**：
  状态：`已修复`（2026-06-22，移动断点下锚定回卡片右上角）
  文件：`src/app/globals.css:7580-7587`。
