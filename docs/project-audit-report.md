...
**4. 仓库卫生**
状态：`处理中`（2026-06-27，本地忽略产物清理已处理；HTML 应用拆分仍后续处理）
...
| P2 | 架构 | 鉴权分散、`.js` 引擎绕过类型、仓库产物堆积 | private-tool access 命名、middleware 私有 API 鉴权、`.ts` import 抑制与本地产物清理命令已清；`.js` 引擎 TS 化和 HTML 应用拆分继续拆分 | 各引擎 `.js`、`tsconfig.json`、`private-tool-access`、`scripts/clean-local-artifacts.mjs` |
