...
- **对比度不达标**：`--muted: #b0aea5` 在浅背景上仅约 **2.11:1**（WCAG AA 要求 4.5:1），却被用在几乎所有次要文字——hero 描述、联系方式、导航、文章 meta。
  ...
  - 修复：调暗到 `≈#8a887e`（≥4.5:1），或为正文场景引入更深的 `--text-secondary`。
...
- **`MouseTrail` 性能隐患**：桌面端无视 reduced-motion，每次 mousemove 分配粒子 + `mix-blend-multiply`。文件：`src/components/ui/MouseTrail.tsx:50-116`。
