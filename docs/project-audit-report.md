...
**1. 鉴权逻辑分散、命名误导**
状态：`处理中`（2026-06-27，命名误导已拆到 private-tool access；middleware 统一鉴权仍后续处理）
...
| P2 | 架构 | 鉴权分散、`.js` 引擎绕过类型 | private-tool access 命名与 `.ts` import 抑制已清；middleware 化与 `.js` 引擎 TS 化继续拆分 | 各引擎 `.js`、`tsconfig.json`、`private-tool-access` |
