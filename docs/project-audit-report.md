...
| 2026-06-21 | 安全 P0-2 回归 | 已修复 | 修正上一轮安全头加固的误伤：全站默认仍用 `frame-ancestors 'none'` / `X-Frame-Options: DENY` 防外站嵌入，但 `/tools/margin-analysis/*` 作为同源 iframe 静态财务工具改为 `frame-ancestors 'self'` / `X-Frame-Options: SAMEORIGIN`，恢复 `/finance/margin-analysis` 内嵌工具。 | `node --test tests/security-contract.test.mjs`；`npx tsc --noEmit`；`npm run lint`；`git diff --check`；`npm run test:site`；`npm run build:vercel`；本地生产 `curl -I -L http://localhost:3024/tools/margin-analysis/index.html` 已确认 `SAMEORIGIN`，`/finance/margin-analysis` 仍为 `DENY`；Playwright 确认 iframe 内容渲染 |
...
- 回归修复（2026-06-21）：全站默认禁止外部嵌入不变；`/tools/margin-analysis/*` 是 `/finance/margin-analysis` 的同源 iframe 工具路径，需使用 `frame-ancestors 'self'` / `X-Frame-Options: SAMEORIGIN`，避免被 P0-2 安全头误伤。
...
- **`ChatWidget` 模态 a11y 缺失**：无 `role="dialog"` / `aria-modal`、无 focus trap、无 Escape 关闭、无 `aria-live`。文件：`src/components/ChatWidget.tsx:818-980`。
