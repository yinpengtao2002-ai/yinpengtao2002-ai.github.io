...
| 2026-06-30 | UI P1-10y | 已修复 | 已继续收口首页 hero 模型舞台阴影 token 使用：`.home-hero-stage-panel`、`.home-hero-stage-preview` 和 `.home-hero-stage-skeleton-window` 不再直接写 `rgba(20, 20, 19, ...)` 阴影，改读 `--home-hero-stage-panel-shadow`、`--home-hero-stage-preview-shadow` 和 `--home-hero-stage-skeleton-window-shadow`，并从 `--foreground` 派生。此项只关闭模型舞台面板 / 预览 / 骨架窗口阴影，不代表其它首页装饰阴影已全部 token 化。 | `node --test tests/design-token-contract.test.mjs`（先失败于旧 stage 阴影，后 26/26 通过）；`npx tsc --noEmit`；`git diff --check`；`npm run lint`；`npm run test:site`（349/349）；`npm run build:vercel`；本地生产 Playwright `/` 桌面与 `390×844` 均确认舞台面板、预览框和骨架窗口可见，阴影从 root token 解析，无横向溢出，console error=0。 |
| 2026-06-30 | UI P1-10z | 已修复 | 已继续收口首页 hero 模型舞台控件阴影 token 使用：`.home-hero-stage-float`、`.home-hero-stage-tab` 和 tab hover / focus / active 态不再直接写 `rgba(20, 20, 19, ...)` 阴影，改读 `--home-hero-stage-float-shadow`、`--home-hero-stage-tab-shadow` 和 `--home-hero-stage-tab-active-shadow`，并从 `--foreground` 派生。此项只关闭模型舞台浮动指标卡和切换 tab 阴影，不代表其它首页装饰阴影已全部 token 化。 | `node --test tests/design-token-contract.test.mjs`（先失败于旧 stage 控件阴影，后 27/27 通过）；`npx tsc --noEmit`；`git diff --check`；`npm run lint`；`npm run test:site`（350/350）；`npm run build:vercel`；本地生产 Playwright `/` 桌面与 `390×844` 均确认浮动卡、普通 tab 和 active tab 可见，阴影从 root token 解析，无横向溢出，console error=0。 |
| 2026-06-30 | UI P1-10aa | 已修复 | 已继续收口首页 hero 左侧浮动 mini widgets 的阴影 token 使用：`.home-mini-widget` 不再直接写 `rgba(20, 20, 19, 0.07)` 阴影，改读 `--home-mini-widget-shadow`，并从 `--foreground` 派生。此项只关闭当前首页桌面实际渲染的两个装饰 mini widget 阴影，不代表 mini widget 内部色点或其它首页装饰色已全部 token 化。 | `node --test tests/design-token-contract.test.mjs`（先失败于旧 mini widget 阴影，后 28/28 通过）；`npx tsc --noEmit`；`git diff --check`；`npm run lint`；`npm run test:site`（351/351）；`npm run build:vercel`；本地生产 Playwright `/` 桌面确认两个 mini widget 可见、阴影从 `--home-mini-widget-shadow` 解析、无横向溢出；`390×844` 移动端确认 mini widgets 按既有响应式规则隐藏、无横向溢出，console error=0。 |
| 2026-06-30 | UI P1-10ab | 已修复 | 已继续收口首页 hero 左侧浮动 mini widgets 的可见 accent 色：`.home-mini-widget-chrome span` 与 `.home-mini-widget-bars span` 不再直接写 `#dc7f5f` / `#e8c66d` / `#7ebc9a` / `#d9785c` / `#6f9eb8` / `#90a675`，改读 `--home-mini-widget-accent-*` 并从站点 accent token 派生。此项只关闭当前首页桌面实际渲染的 mini widget 窗口点和柱条颜色，不代表未渲染备用 dots/status 色点或其它首页装饰色已全部 token 化。 | `node --test tests/design-token-contract.test.mjs`（先失败于旧 mini widget hex 色值，后 29/29 通过）；`npx tsc --noEmit`；`git diff --check`；`npm run lint`；`npm run test:site`（352/352）；`npm run build:vercel`；本地生产 Playwright `/` 桌面确认 mini widget 窗口点和柱条颜色均从 `--home-mini-widget-accent-*` 解析、无横向溢出；`390×844` 移动端确认 mini widgets 按既有响应式规则隐藏、无横向溢出，console error=0。 |
| 2026-06-30 | UI P1-10ac | 已修复 | 已继续收口首页“工具与思考”真实渲染路径的视觉卡片阴影：`.home-thinking-visual-card` 不再直接写 `rgba(20, 20, 19, 0.08)` 阴影，改读 `--home-thinking-visual-card-shadow`，并从 `--foreground` 派生。此项只关闭该视觉卡片阴影，不代表图片遮罩、预览面板、count pill 或 track 卡片阴影已全部 token 化。 | `node --test tests/design-token-contract.test.mjs`（先失败于旧 visual card 阴影，后 30/30 通过）；`npx tsc --noEmit`；`git diff --check`；`npm run lint`；`npm run test:site`（353/353）；`npm run build:vercel`；本地生产 Playwright `/` 桌面确认 visual card 可见、阴影从 `--home-thinking-visual-card-shadow` 解析、无横向溢出；`390×844` 移动端确认 visual card 可见、无横向溢出，console error=0。 |
...
- **设计 token 形同虚设**：`--space-*/--radius-*/--shadow-*` 定义了却几乎没人用（全文仅引用 3 次），实际全是裸值（`8px` 出现 47 次、16 种硬编码阴影）。文件：`src/app/globals.css:7729-7768`。
  状态：`处理中`（2026-06-30，已先收口共享财务工具返回按钮、财务工具页外层壳、全站 selection、全局导航阴影、首页模型舞台 accent、首页工具与思考 track accent、首页 hero intro / 模型入口阴影、首页 hero 模型舞台阴影、首页 hero 模型舞台控件阴影、首页 hero 浮动 mini widgets 阴影和 accent 色、全站 AI 助手外壳、财务模型状态丝带、财务 AI 表面、财务 AI 上传工作台、财务 AI 上传动作按钮、财务 AI 头像、财务 AI 空态标签、财务 AI composer 控件、财务 AI thinking chips、财务 AI 明细筛选按钮、财务 AI 明细筛选菜单、财务 AI 明细筛选复选框、财务 AI 明细表斑马纹、财务 AI 用户消息气泡、财务 AI 空态预览瀑布图连接线、财务 AI 错误文案和财务 AI 明细表表头二十八条真实渲染路径）
  ...
  - 处理记录（2026-06-30）：首页 hero 模型舞台 `.home-hero-stage-panel`、`.home-hero-stage-preview` 和 `.home-hero-stage-skeleton-window` 原先仍直接写三处 `rgba(20, 20, 19, ...)` 阴影；本轮新增 `--home-hero-stage-panel-shadow`、`--home-hero-stage-preview-shadow` 和 `--home-hero-stage-skeleton-window-shadow`，让舞台面板、预览框和加载骨架窗口阴影从 `--foreground` 派生，并用 `tests/design-token-contract.test.mjs` 锁定不回到硬编码阴影。其它首页装饰阴影和色值继续后续逐项评估。
  - 处理记录（2026-06-30）：首页 hero 模型舞台控件 `.home-hero-stage-float`、`.home-hero-stage-tab` 和 tab hover / focus / active 态原先仍直接写三处 `rgba(20, 20, 19, ...)` 阴影；本轮新增 `--home-hero-stage-float-shadow`、`--home-hero-stage-tab-shadow` 和 `--home-hero-stage-tab-active-shadow`，让浮动指标卡和切换 tab 阴影从 `--foreground` 派生，并用 `tests/design-token-contract.test.mjs` 锁定不回到硬编码阴影。其它首页装饰阴影和色值继续后续逐项评估。
  - 处理记录（2026-06-30）：首页 hero 左侧装饰 mini widgets `.home-mini-widget` 原先仍直接写 `rgba(20, 20, 19, 0.07)` 阴影；本轮新增 `--home-mini-widget-shadow`，让当前首页实际渲染的两个 mini widget 阴影从 `--foreground` 派生，并用 `tests/design-token-contract.test.mjs` 锁定不回到硬编码阴影。mini widget 内部红 / 黄 / 绿 / 蓝色点和其它首页装饰色继续后续逐项评估。
  - 处理记录（2026-06-30）：首页 hero 左侧装饰 mini widgets 的窗口点和柱条原先仍直接写六个 hex 色值；本轮新增 `--home-mini-widget-accent-primary`、`--home-mini-widget-accent-secondary`、`--home-mini-widget-accent-tertiary` 和 `--home-mini-widget-accent-warm`，让当前首页实际渲染的 `.home-mini-widget-chrome span` 与 `.home-mini-widget-bars span` 色值从站点 accent token 派生，并用 `tests/design-token-contract.test.mjs` 锁定不回到硬编码 hex。未渲染备用 dots/status 色点和其它首页装饰色继续后续逐项评估。
...
| P1 | UI | token / 原语库零采用、死代码 | 死代码、旧原语库已清理；共享财务返回按钮、财务工具页外壳、全站 selection、导航阴影、首页模型舞台 accent、首页 hero intro / 模型入口阴影、首页 hero 模型舞台阴影、首页 hero 模型舞台控件阴影、首页工具与思考 track accent、全站 AI 助手外壳、财务模型状态丝带、财务 AI 表面、财务 AI 上传工作台、财务 AI 上传动作按钮、财务 AI 头像、财务 AI 空态标签、财务 AI composer 控件、财务 AI thinking chips、财务 AI 明细筛选按钮、明细筛选菜单、明细筛选复选框、明细表斑马纹、用户消息气泡、空态预览瀑布图连接线、错误文案和明细表表头已 token 化，其他真实渲染路径继续逐项收口 | `ToolBackButton.tsx`、`SiteNavigation.tsx`、`HeroModelStage.tsx`、`HomeThinkingSection.tsx`、`ChatWidget.tsx`、`ui/*`、`globals.css`、`lib/animations*` |
