...
- `销量` 是默认体量分母；`销量` 后面的数值列会作为财务指标，例如净收入、成本、边际，也可以替换为质量、费用或单车指标。`margin-analysis` 的下载模板额外提供 `指标角色` 行，可把任意一列标成 `分母`，把一个或多个指标列标成 `分子`，用于毛利/净收入等分子/分母口径；没有角色行时仍按旧规则兼容识别。单车归因要求每行的业务维度填写完整；销量、收入、成本、边际等数值允许为 0，全零业务行仍会保留。行级分母/销量为空时继续按 0 处理，用于附属金额行并入同维度汇总。
