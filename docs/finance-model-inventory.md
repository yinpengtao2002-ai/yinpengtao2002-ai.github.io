...
- 预算/实际类模板必须优先使用独立工作表表达对比口径；不要把预算和实际做成经营明细里的可填行项目。
...
- 图表：经营指标卡、按毛利和 EBIT 分组的年度柱状图、利润瀑布、车型贡献、情景利润柱状图及年度折线图。经营参数是一行横栏，向下展开调参、收起保留结果；顶部七项导航滚动时固定，各页大标题移除。损益表压缩行距、去掉逐项原表行号，保留点击科目核查来源；情景比较为独立页面。新增项目与车型卡片、单车EBIT/利润贡献图、亏损组合核查，以及保本销量、安全边际、覆盖倍数和年度费用覆盖。各经营页面共享参数与筛选，数据与口径放最后。只保留底表基线与用户调整后两种实时口径，取消系统预设参数组合；用户主动保存的情景继续可比。五项核心指标卡分色呈现主值、关联业务量、基线和中性的增减差额，不评价方案好坏。
- 明细：损益明细页保留分车型、MIX、Sales 至 EBIT 完整损益表、来源核查与导出。
...
- 上传前检查数值型业务字段；非空非法值会显示 Sheet、行号和列名并阻止计算，万/亿等单位后缀按真实倍率转换。
- 模板族为 `operating-detail`，Excel 模板通过 `实际`、`预算` 两个工作表表达对比口径，上传后系统内部再归一化出实际/预算口径；固定科目仍由本模型的科目区维护。
- 支持示例数据和 CSV/XLSX 模板下载。
- 左侧将「下钻顺序」和「维度钻取」拆成两个轻量控制块；可拖动维度胶囊，或用首位 / 上移 / 下移 / 末位按钮调整顺序，键盘移动后会播报新位置。
...
- 用途：比较两期单车指标变化，并拆解为结构效应和费率效应；默认以销量为分母，也支持通过模板角色行分析毛利/净收入等分子/分母型指标。

已设计图表：

- 每个启用下钻层级都会动态生成一张归因瀑布图。
- 瀑布图展示基期指标、各维度成员总贡献、可选「其他」项和当期指标；用户可选择单车/单位指标或比率指标，比率指标按分子/分母展示并使用百分比。
- 每个层级配套一张可折叠明细表，包含基期/当期分母、基期/当期占比、基期/当期指标、结构效应、费率效应和总贡献。

交互模式：

- 静态工具通过 `public/tools/margin-analysis/index.html` iframe 加载。
- 支持上传 CSV/XLS/XLSX 或加载示例数据。
- 支持 CSV/XLSX 示例格式下载；模板族为 `operating-detail`，用同一经营明细事实表选择基期和当期做归因，单车归因不需要填写预算/实际口径。
- CSV 使用 RFC 4180 状态机，支持 BOM、CRLF、单元格内换行、转义引号和尾部空列，并限制文件、行、列和单元格规模。
- 下载模板包含 `指标角色` 行：默认销量为分母，净收入、成本、边际为分子；用户可改成净收入分母、毛利分子等口径，旧版没有角色行的数据仍兼容识别。
- 期间统一规范为 `YYYY-MM`，支持日期单元格及常见年月写法；默认选择数据中最近两期。
- 月份和每个业务维度在计算前必须填写完整；分母和指标列区分合法、空白、非法，明确填写的 0 在销量、收入、成本、边际等列中均为合法值，全零业务行也会保留。行级分母/销量空白继续按 0 处理，允许有金额的附属行并入同维度汇总；上传问题会用居中弹窗显示来源行列，不再只在侧栏短暂提示。
- 选择单位名称、分析指标、显示格式和归因模式；显示格式支持常规数值和百分比。
- 可拖动维度卡片，或用首位 / 上移 / 下移 / 末位按钮调整下钻层级；键盘移动后会播报新位置，也可移除不需要的维度。
...
- 公开主接口不使用私有工具访问码；旧 Finance AI 专用 Access Route 已移除。AI API 使用生产 Upstash 限流、稳定公开错误契约和 Primary GPT → DeepSeek fallback 顺序。
- 模板族为 `operating-detail`，与趋势、利润质量、BI 和预算实际共用经营明细事实表；Excel 模板通过工作表名称表达实际、预算、目标或预测，系统内部再归一化为分析口径。
- 上传前是左侧拖拽/按钮上传工作台，保留上传、下载示例格式和查看只读示例三个入口；桌面右侧展示三张示例图表缩略预览，手机端不展示这些预览图；底部输入条只保留“等待上传”小状态，不在上传卡里展示额外数据识别状态块，模拟对话只在只读示例页展示。
- 上传数据只保存在当前浏览器页面会话中，刷新后清空。
- 用户在聊天输入框中直接用自然语言提问。
- AI 先生成可执行分析计划，前端再基于上传数据做确定性聚合和计算，最后由 AI 解读计算结果。
- 正式助手 API 只支持 `plan` 和 `explain` 分析链路；上传原表不直接发送给 AI 模型，旧的直传底稿分析、AI 取数请求和原始切片分析模式不再保留。
...
- 支持选择最多五个诊断维度作为组合粒度。
- 支持选择质量指标、参考指标和期间范围。
- 字段角色不再由销量列位置决定；已知比率按分子/分母重算，期末指标取最新期间值，NPS、满意度等缺少底层计数的指标不生成合计并显示警告。
- 必填期间和数值在计算前校验；括号负数、百分号、万/亿倍率按共享口径解析。
- 支持自动生成维度筛选器并重置筛选。
- 解锁后数据只保存在当前页面会话中。
- 移动端沿用折叠式控制台模式。
