...
- 图表：经营指标卡、按毛利和 EBIT 分组的年度柱状图、利润瀑布、车型贡献、情景利润柱状图及年度折线图。经营参数是一行横栏，向下展开调参、收起保留结果；顶部七项导航滚动时固定，各页大标题移除。损益表压缩行距、去掉逐项原表行号，保留点击科目核查来源；情景比较为独立页面。新增项目与车型卡片、单车EBIT/利润贡献图、亏损组合核查，以及保本销量、安全边际、覆盖倍数和年度费用覆盖。各经营页面共享参数与筛选，数据与口径放最后。
