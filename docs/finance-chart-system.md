...
- `margin-analysis` 已使用共享 RFC 4180 解析、最近两期默认选择、数值质量阻断和 `Map` 聚合；月份及每个业务维度均为必填，销量、收入、成本、边际等数值明确填写 0 时合法且全零业务行不被删除。行级分母/销量空白按 0 处理并允许金额并入同维度汇总，非法数值才阻断；上传校验错误通过居中弹窗展示来源行列，零销量单位指标返回未定义状态，通用指标展示不再假设人民币。
