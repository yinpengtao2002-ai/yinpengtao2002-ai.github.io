...
- `src/lib/finance/charts/`：统一 `FinanceChartSpec`、Plotly 主题、图表尺寸、移动端规则、瀑布桥、趋势图、热力图、气泡散点图、横向排名图、系列柱状图和明细表协议。
...
- 改 `FinanceChartSpec`、主题、颜色、单位缩放、移动端密度或 Plotly config：同步检查所有使用同一图表族的模型页面、演示页、预览图和图表合同测试。
...
2. 下一步：抽出 `waterfall-bridge`，让财务 AI、敏感性、预算实际先共用同一套桥图规格。
3. 抽出 `pvm.ts`，让单车归因和财务 AI 使用同一套结构效应 / 费率效应公式。
4. 抽出热力图、气泡散点、横向排名和 KPI 卡的模型接入规则。
5. 最后整理字段识别、上传解析和模型清单生成逻辑。
