...
test("margin analysis iframe carries a static asset version", () => {
  assert.match(marginAnalysisPage, /MARGIN_ANALYSIS_TOOL_VERSION\s*=\s*"20260816-zero-value-validation"/);
  assert.match(
    marginAnalysisPage,
...
test("goalkeeper landscape game is exposed as a thinking lab tool", async () => {
  ...
  const gameManifest = JSON.parse(await readFile(
    new URL("../public/tools/goalkeeper-landscape/manifest.webmanifest", import.meta.url),
    "utf8"
  ));
  const gameScriptPath = new URL("../public/tools/goalkeeper-landscape/assets/index-CYqDptrR.js", import.meta.url);
  const gameScript = await stat(gameScriptPath);
  const gameScriptSource = await readFile(gameScriptPath, "utf8");
  const gameStylePath = new URL("../public/tools/goalkeeper-landscape/assets/index-C3Y5dsvI.css", import.meta.url);
  ...
  assert.match(goalkeeperRuntime, /index-CYqDptrR\.js/);
  ...
  assert.match(goalkeeperPage, /index-C3Y5dsvI\.css/);
  assert.match(goalkeeperPage, /开始后自动横屏/);
  assert.match(goalkeeperPage, /screen-orientation/);
  assert.match(goalkeeperPage, /x5-orientation/);
  assert.match(goalkeeperPage, /x5-fullscreen/);
  assert.match(goalkeeperPage, /<GoalkeeperLandscapeRuntime \/>/);
  assert.match(goalkeeperPage, /id="saveAssistSwitch"/);
  assert.match(goalkeeperPage, /守门挑战/);
  assert.doesNotMatch(goalkeeperPage, /弹力手套/);
  ...
  assert.match(gameIndex, /\/tools\/goalkeeper-landscape\/assets\/index-CYqDptrR\.js/);
  assert.match(gameIndex, /\/tools\/goalkeeper-landscape\/assets\/index-C3Y5dsvI\.css/);
  assert.match(gameIndex, /\/tools\/goalkeeper-landscape\/manifest\.webmanifest/);
  assert.equal(gameManifest.display, "fullscreen");
  assert.equal(gameManifest.orientation, "landscape");
  ...
  assert.match(gameScriptSource, /psg-midnight-match-ball-texture/);
  assert.match(gameScriptSource, /psg-midnight-navy-university-red-white/);
  assert.match(gameScriptSource, /anatomical-two-segment-wrap-and-web/);
  assert.match(gameScriptSource, /retina-close-up-512/);
  assert.match(gameScriptSource, /adaptive-retina-render-target/);
  assert.match(gameScriptSource, /quiet-crowd-scoreboard-training-backdrop/);
  ...
  assert.match(gameScriptSource, /cohesive-goalkeeper-training-lane/);
  assert.match(gameScriptSource, /stray-ball-containment-padded-rails/);
  assert.match(gameScriptSource, /training-lane-safety-barrier/);
  assert.match(gameScriptSource, /purpose-built-launcher-service-zone/);
  assert.match(gameScriptSource, /training-lane-ball-rack/);
  assert.match(gameScriptSource, /training-lane-control-cabinet/);
  assert.match(gameScriptSource, /cohesive-autonomous-ball-launcher/);
  assert.match(gameScriptSource, /feed-aim-launch-without-operator-clutter/);
  ...
  assert.match(gameScriptSource, /launcher-protective-roll-frame/);
  assert.match(gameScriptSource, /professional-freestanding-training-goal-kit/);
  assert.match(gameScriptSource, /integrated-white-tube-return-frame/);
  assert.match(gameScriptSource, /integrated-white-return-frame/);
  assert.doesNotMatch(gameScriptSource, /goal-net-support-post/);
  assert.doesNotMatch(gameScriptSource, /goal-net-support-cable/);
  assert.doesNotMatch(gameScriptSource, /goal-net-ground-rope/);
  assert.match(gameScriptSource, /slim-white-portable-goal-frame/);
  assert.match(gameScriptSource, /clean-training-ground-goal-equipment/);
  assert.doesNotMatch(gameScriptSource, /goal-net-cage-seam/);
  assert.match(gameScriptSource, /goal-frame-return-/);
  ...
  assert.match(gameScriptSource, /integrated-frame-bound-net/);
  assert.match(gameScriptSource, /naturally-sagged-square-training-net/);
