...
test("goalkeeper landscape game is exposed as a thinking lab tool", async () => {
  ...
  const gameScriptPath = new URL("../public/tools/goalkeeper-landscape/assets/index-CNSyQgDE.js", import.meta.url);
  ...
  assert.match(goalkeeperRuntime, /index-CNSyQgDE\.js/);
  ...
  assert.match(gameIndex, /\/tools\/goalkeeper-landscape\/assets\/index-CNSyQgDE\.js/);
  ...
  assert.match(gameScriptSource, /clean-non-grass-training-floor-depth-shadows/);
  ...
  assert.match(gameScriptSource, /field-surface-panel/);
  assert.doesNotMatch(gameScriptSource, /field-turf-maintenance-brush/);
  assert.match(gameScriptSource, /clean-training-floor-scuff-and-line-kit/);
  assert.doesNotMatch(gameScriptSource, /field-edge-tuft-cluster/);
  assert.match(gameScriptSource, /field-floor-scuff/);
  assert.match(gameScriptSource, /field-boot-scuff/);
