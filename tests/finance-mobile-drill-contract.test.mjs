...
test("Perspective BI default demo data is rich enough to exercise chart exploration", () => {
  ...
  assert.match(perspectiveEngine, /回款金额/);
  assert.match(perspectiveEngine, /期初库存/);
  assert.match(perspectiveEngine, /期末库存/);
  assert.match(perspectiveEngine, /最高库存/);
  assert.match(perspectiveEngine, /最低库存/);
  const sampleBlockMatch = perspectiveEngine.match(/function buildSampleRows\(\) \{([\s\S]*?)\n\}/);
  assert.ok(sampleBlockMatch, "demo data should be generated by a dedicated sample builder");
  assert.doesNotMatch(sampleBlockMatch[1], /单车净收入|单车边际|预算达成率/);
});
