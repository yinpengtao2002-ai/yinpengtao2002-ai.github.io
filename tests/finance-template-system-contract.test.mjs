...
test("finance template hub records the template families used by the model suite", async () => {
  ...
  assert.deepEqual(
    ...
    ["operating-detail", "budget-actual", "unit-attribution", "profit-sensitivity"]
  );

  assert.deepEqual(
    getFinanceTemplateFamilyForModel("monthly-trend").modelSlugs,
    ["monthly-trend", "profit-structure", "perspective-bi", "finance-ai-assistant"]
  );
  assert.equal(getFinanceTemplateFamilyForModel("business-analysis").slug, "budget-actual");
  assert.equal(getFinanceTemplateFamilyForModel("margin-analysis").slug, "unit-attribution");
  assert.equal(getFinanceTemplateFamilyForModel("sensitivity-analysis").slug, "profit-sensitivity");
});
...
test("finance template center is documented next to chart and interaction centers", async () => {
  ...
  assert.match(templateSystem, /\| business-analysis \| budget-actual \|/);
  assert.match(templateSystem, /\| margin-analysis \| unit-attribution \|/);
  assert.match(templateSystem, /\| sensitivity-analysis \| profit-sensitivity \|/);
