...
test("home page has an explicit continue cue for below-the-fold content", () => {
  ...
  assert.match(hero, /精选 4 个 · 共 7 个模型/);
...
test("home hero uses an interactive model stage instead of a static question grid", () => {
  ...
  const stageTabRule = cssRule(".home-hero-stage-tab {");
  const stageTabModelRule = cssRule(".home-hero-stage-tab-model {");
  assert.match(stageTabRule, /align-content:\s*center/);
  assert.match(stageTabModelRule, /font-weight:\s*600/);
  assert.match(stageTabModelRule, /font-size:\s*10px/);
  assert.doesNotMatch(stageTabModelRule, /font-weight:\s*800/);
...
test("home thinking count pills remain legible on the image card and category cards", () => {
  assert.match(globals, /\.home-thinking-preview-panel \.home-thinking-count-pill\s*\{[\s\S]*background:\s*rgba\(255,\s*250,\s*240,\s*0\.92\)[\s\S]*color:\s*color-mix\(in srgb,\s*var\(--thinking-track-accent,\s*#fffaf0\)\s*70%,\s*var\(--foreground\)\)[\s\S]*box-shadow:\s*var\(--home-thinking-preview-count-pill-shadow\)/s);
